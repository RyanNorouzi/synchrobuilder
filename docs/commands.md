...
file) and for the plugin's skills (`scripts/gen-skills.mjs` generates one
`SKILL.md` per command from it, so the two cannot drift).
...
"available" means the command is implemented and covered by tests in this
