...
export async function run({ flags = {}, cwd = process.cwd(), stdout = process.stdout, stderr = process.stderr, stdin = process.stdin, platform = process.platform, fingerprint = null }) {
  const json = Boolean(flags.json);
  const modules = await loadManifestModules();
  const raw = modules.readManifest(cwd);
  if (!raw) { stderr.write(`No synchrobuilder.json found in ${cwd}. Run "synchrobuilder init" first to create one.\n`); return 2; }
  const v = modules.validateManifest(raw);
  ...
  const fp = fingerprint || await loadFingerprint({ cwd, platform });
  ...
  const steps = buildPlan({ manifest, fingerprint: fp, platform, composeFile: detectComposeFile(cwd), envPresent, tokenize: modules.tokenizeCommand });
  ...
  const { results, failed, background } = await executeSteps(steps, { cwd, platform, yes: Boolean(flags.yes), json, stdout, stderr, stdin });
  let health;
  if (failed) health = { ok: false, evidence: [`setup stopped at "${results[results.length - 1].id}" before the health check could run`] };
    ...
    health = await runHealthCheck(manifest.healthCheck, { cwd, platform, tokenize: modules.tokenizeCommand });
